    ...
    tabPanel("Without Date Data",
             ...
             fluidRow(
               ...
               column(4,
                      h3("Set up inputs"),
                      p("Warning: do not upload personally identifiable information (PII)", style="color:red"),
                      fileInput("noTimeTab", "Table of with grouped list(s) of people (required)", accept=c(".xlsx", ".csv")),
                      br(),
                      br(),
                      h3("Set up outputs"),
               ...
               column(4,
                      ...
                      helpText("Strength options will be applied to all columns unless user selects custom strength specified."),
                      radioButtons("noTimeStrength", "Strength of link",
                                   choiceNames = list("Definite epi link", "Probable epi link", "Possible epi link",
                                                      "No strength specified (strength left blank)", "Column-specific strength"),
                                   choiceValues = list("definite", "probable", "possible", "none", "custom"),
                                   selected = "probable"),
                      column(2),
                      column(10,
                             fileInput("noTimeCustomStrength", tags$div("Custom strength table", tags$br(), 
                                                                        tags$p("Valid strength values are Definite, Probable, Possible, or strength left blank.", style="font-size: 85%; font-weight:100;")), 
                                       accept=c(".xlsx", ".csv")))),
               column(2)),
